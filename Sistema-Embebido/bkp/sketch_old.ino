...
double readFlex (const int, const int, const int, const int);
int convertAngleIntoPosition (const float);
int convertInformationIntoMovement (const int, const int, const int, const int);
...
void loop()
{
  ...
  if ((unsigned long)(currentMillis - previousMillis) >= interval)
  {
    /*if(bluetooth.available())
    {
      ...
      Serial.println(previousMovement);
      ...
      {
        Serial.println("El movimiento es nuevo!");  
     //   sendInformationViaBluetooth (newMovement);
      ...
      {
        Serial.println("El movimiento NO es nuevo!");  
...
double readFlex (const int sensorName, const int straightResistance, const int bendResistance, const int divisorResistance)
{
  ...
  double angleValue = map(sensorValue, straightResistance, bendResistance, 0, 90.0);
...
int convertAngleIntoPosition (const float flexAngle)
{
  ...
  if (flexAngle < 40.0)
  {
    flexPosition = (int) STRAIGHT_FLEX;
  }
  else if (flexAngle > 84.0)
  { 
    flexPosition = (int) BEND_FLEX;
  }
  else
  {
    flexPosition = (int) MEDIUM_BEND_FLEX;
  }
